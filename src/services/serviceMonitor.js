...
class ServiceMonitor {
  ...
  loadConfig() {
    try {
      const configPath = path.join(process.cwd(), 'config', 'service-monitor.json');
    ...
    return {
      enabled: true,
      checkInterval: 10000,
      services: ['apollo-miner', 'node', 'ckpool', 'apollo-api', 'apollo-ui-v2'],
      logLevel: 'info',
      autoStart: true,
  ...
  async start() {
    ...
    if (this.isDevelopment()) {
      console.log('Service monitor skipped in development environment - status managed by API calls only');
      return;
    }
    ...
    console.log(`Service monitor started with interval of ${this.checkInterval}ms`);
  }
  ...
  async checkServiceStatus(serviceName) {
    try {
      ...
      if (this.isDevelopment()) {
        console.log(`Skipping systemd check for ${serviceName} in development environment`);
        return { 
          serviceName, 
          status: 'unknown', 
      ...
      await this.updateServiceStatus(dbServiceName, mappedStatus, requestedStatus);
      
      return { serviceName: dbServiceName, status: mappedStatus, systemdStatus: status };
    } catch (error) {
      console.error(`Error checking service ${serviceName}:`, error.message);
      // If service doesn't exist or there's an error, mark as unknown
      const dbServiceName = this.getDatabaseServiceName(serviceName);
      await this.updateServiceStatus(dbServiceName, 'unknown', null);
      return { serviceName: dbServiceName, status: 'unknown', systemdStatus: 'error' };
    }
  }
  ...
  async checkAllServices() {
    try {
      ...
      if (this.config.logLevel === 'debug') {
        console.log('Service status updated:', results.map(r => `${r.serviceName}: ${r.status}`));
      }
      return results;
    } catch (error) {
    ...
    try {
      ...
      if (existing) {
        ...
        if (existing.status !== status) {
          ...
          console.log(`Service ${serviceName} updated: ${existing.status} -> ${status}`);
        } else {
        ...
        console.log(`New service ${serviceName} registered with status: ${status}`);
      }
    } catch (error) {
  ...
  getRemoteNodeInfo() {
    ...
    return {
      ...
      message: `Monitoring remote node at ${process.env.BITCOIN_NODE_HOST}:${process.env.BITCOIN_NODE_PORT || 8332}`
    };
  }
}
